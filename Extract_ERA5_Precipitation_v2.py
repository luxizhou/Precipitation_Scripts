...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 22 11:37:55 2021
...
ERA5_folder='/media/lzhou/Extreme SSD/Precipitation/ERA5/ERA5_prec'
...
era5_files.sort()
#%%
for era5_file in era5_files[476:477]:
    print(era5_file)
    era5_file = os.path.join(ERA5_folder,era5_file)
    #%%
    year = int(era5_file[-14:-10])
    doy_offset = datetime(year,8,31)
    
    data = xr.open_dataset(era5_file) # read hourly precipitation from ERA5 file (unit:m)
    data['tp'] = data['Rainf']*3.6  #convert rain flux (kg/(m2s) to m)
    data=data.drop_vars('Rainf')
    ...
    era_lats = era5_daily.lat.values
    era_lons = era5_daily.lon.values
...
#%%
    for cmaid in cmaids:
        ...
        bounds = gpd.GeoSeries(cascaded_union(polygons),crs=proj_epsg)
        ...
        idlat1 = np.argmin(abs(era_lats-minlat))
        idlat2 = np.argmin(abs(era_lats-maxlat))
        #%%
        dummy = era5_daily[dict(date=1, lat=np.arange(idlat1-1,idlat2+1), \
...
#%%
        ...
        for name, group in grouped:
            ...
            bounds1 = gpd.GeoSeries(cascaded_union(polygons),crs=proj_epsg)
        ...
        for (ii,jj) in zip(days,doys):
            ...
            #print(ii,jj)
            precip = era5_daily[dict(date=jj, lat=np.arange(idlat1-1,idlat2+1), \
                                     lon=np.arange(idlon1-1,idlon2+1))]
        ...
        precip_daily = precip_daily.fillna(0.)    
...
#        precip_total.to_pickle(os.path.join(Output_folder, case, total_file))
    ...
    #                  cmap='Blues', scheme='naturalbreaks', cax=cax, \
        ...
        ax.title.set_text('ERA5 ' + df.Name.iloc[0] + ' ' + days[0] + '-' + days[-1])
        ax.set_xlim([70,140])
        ax.set_ylim([15,55])    
