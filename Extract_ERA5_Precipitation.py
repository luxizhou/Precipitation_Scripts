...
era5_files.sort()
for era5_file in era5_files[1:2]:
    print(era5_file)
    ...
    for cmaid in cmaids:
        ...
        dummy = era5_daily[dict(date=1, latitude=np.arange(idlat1-1,idlat2+1), \
                                longitude=np.arange(idlon1-1,idlon2+1))]
        ...
        for (ii,jj) in zip(days,doys):
            ...
            #print(ii,jj)
            precip = era5_daily[dict(date=jj, latitude=np.arange(idlat1-1,idlat2+1), \
                                     longitude=np.arange(idlon1-1,idlon2+1))]
        ...
        precip_daily.to_pickle(os.path.join(Output_folder, 'ERA5_1500km_12', daily_file))
        precip_total.to_pickle(os.path.join(Output_folder, 'ERA5_1500km_12', total_file))
    ...
    #                  cmap='Blues', scheme='naturalbreaks', cax=cax, \
        ...
        fig.savefig(os.path.join(Output_folder,'ERA5_1500km_12', figname))
        plt.close(fig)
